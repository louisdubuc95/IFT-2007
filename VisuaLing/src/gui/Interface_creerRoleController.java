 ...
 * @author antoinehudon
 */
public class Interface_creerRoleController implements Initializable {
    ...
    private void btnEnregistrerAction(ActionEvent event) {
        String nom_role = txtNom.getText();
        Stage window = (Stage) btnEnregistrer.getScene().getWindow();
        try
        {
            m_controller.addRole(nom_role);
            parentController.addRoleComboBox(nom_role);
            window.close();
        }
        catch(Exception e)
        {
